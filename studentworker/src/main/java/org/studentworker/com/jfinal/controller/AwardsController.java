...
public class AwardsController extends BaseController{
	...
	public void list(){
		...
		Page<Model> page = Awards.dao.paginate(pageNumber, pageSize, "select * ", "from awards where del_time is null order by id desc");
	...
	public void edit(){
		...
		if("update".equals(op)){
			Awards rd = new Awards();
			rd.BindModel(this, rd);
			rd.set("date",DateUtil.dateFormat(getPara("date"), "yyyy-MM-dd"));
			...
			Model m = Awards.dao.findById(id);
			m.set("date", DateUtil.format(m.getLong("date"), "yyyy-MM-dd"));
			setAttr("rd", m);
			render("edit.html");
		}else {
	...
	public void del(){
		int id = getParaToInt("id");
		if(id>0){
			Model del = Awards.dao.findById(id);
	...
	public void payment_edit(){
		...
		if("update".equals(op)){
			...
			int id = getParaToInt("id");
			setAttr("record", InsurancePayment.dao.findById(id));
